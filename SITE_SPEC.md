...
- 6件目を追加する場合は、最古の1件をTOP表示から外す。履歴そのものを削除する必要はない。
- 更新項目は、年月日と内容を表示し、対象ページへ直接リンクする。
- 読み物ページを新規公開した場合は、原則として `/contents/` の一覧にも追加する。

## 読み物一覧の表示ルール
- `/contents/index.html` の `#column` は、記事数の増加を前提として縦方向のコンパクトな一覧とする。
- 各記事は「左に小さなサムネイル、右にカテゴリ・タイトル・説明」の横並びを基本とし、スマホでも同じ構成を維持する。
- 記事は原則として新しいものから上に表示する。
